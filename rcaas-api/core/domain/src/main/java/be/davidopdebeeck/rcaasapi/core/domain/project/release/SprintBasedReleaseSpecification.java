...
import static be.davidopdebeeck.rcaasapi.core.domain.project.ProjectValidationMessages.RESCHEDULING_FROM_DATE_SHOULD_BE_A_RELEASE_DATE;
...
public class SprintBasedReleaseSpecification implements ReleaseSpecification {
    ...
    private SprintBasedReleaseSpecification(Builder builder) {
        ...
        validateReschedulingsStartOnReleaseDates();
    }
    ...
    private void validateReschedulingsStartOnReleaseDates() {
        List<Rescheduling> intermediateReschedulings = new ArrayList<>();

        reschedulings.forEach(reschedulingsToValidate -> {
            SprintBasedReleaseSpecification spec = new Builder()
                .withVersion(version)
                .withStartDate(startDate)
                .withSprintLength(sprintLength)
                .withReschedulings(intermediateReschedulings)
                .build();

            validateConstraints(
                validateTrue(spec.determineReleaseAtDate(reschedulingsToValidate.getFrom()).isPresent())
                    .message(RESCHEDULING_FROM_DATE_SHOULD_BE_A_RELEASE_DATE)
            );

            intermediateReschedulings.add(reschedulingsToValidate);
        });
    }
    ...
    public static final class Builder {
        ...
        public Builder withReschedulings(List<Rescheduling> reschedulings) {
            this.reschedulings = merge(reschedulings);
            return this;
        }
